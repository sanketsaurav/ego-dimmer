...
test("merges defaults without preserving the removed master switch", () => {
  assert.deepEqual(
    JSON.parse(
      JSON.stringify(
        popup.mergeState({
          enabled: false,
          mode: "constrained",
          sites: { "x.com": false, "example.com": true },
        }),
      ),
    ),
    {
...
test("normalizes domains and full URLs", () => {
  ...
  assert.equal(
    popup.normalizeDomain("news.example.com/story"),
    "news.example.com",
  );
...
test("selects the longest matching site entry", () => {
  ...
  assert.deepEqual(
    { ...result },
    { domain: "news.example.com", enabled: true },
  );
  assert.equal(
    popup.findMatchingSite("notexample.com", { "example.com": true }),
    null,
  );
});
...
test("dynamic content-script descriptor uses the shared payload", () => {
  assert.deepEqual(JSON.parse(JSON.stringify(popup.scriptFor("example.com"))), {
